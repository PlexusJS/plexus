...
This is where we define [stateful](/docs/getting-started/concepts#internal-terms) [instances](/docs/getting-started/concepts#internal-terms) things like states, computed, and collections.
...
export const userBroker = api("https://api.example.com/users").auth("bearer", "MySeCrEtToKeN")
```
...
export const startMessageSession = actions(async ({ onCatch }, uuid: string, message?: string) => {
...
export const getFriends = actions(async ({ onCatch }) => {
	// If any part of this action throws an error, it will be caught here
	onCatch(console.error)
