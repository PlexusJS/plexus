---
title: Installation
sidebar_position: 2
---

# Installation

## Quick Start

:::caution Overhaul Incoming.

We are working on a complete overhaul of the <strong>npx script</strong> to be <strong>menu based</strong>, and allow you to do so much more than just install plexus. Excuse it's current <em>state</em>! <small>(get it? hehe)</small>
:::

If you would like to jump in and start working with plexus right away, you can use our npx script to quickly create a core!

<small>With Yarn</small>

```bash prefix="$"
yarn create plexus-core --basic
```

<small>With NPX</small>

```bash
npx create-plexus-core --basic
```

With Typescript...

<small>With Yarn</small>

```bash prefix="$"
yarn create plexus-core --basic --typescript
```

<small>With NPX</small>

```bash
npx create-plexus-core --basic --typescript
```

## Manual Installation

You can also install PlexusJs manually and create your own core structure. See our [Style Guides](/docs/style-guides) for more information.

### Core Package

<small>With Yarn</small>

```bash
yarn add @plexusjs/core
```

<small>With NPM</small>

```bash
npm install --save @plexusjs/core
```

### React Package

<small>With Yarn</small>

```bash
yarn add @plexusjs/core @plexusjs/react
```

<small>With NPM</small>

```bash
npm install --save @plexusjs/core @plexusjs/react
```

### SolidJS Package (WIP ~ doesn't support events)

<small>With Yarn</small>

```bash
yarn add @plexusjs/core @plexusjs/solid
```

<small>With NPM</small>

```bash
npm install --save @plexusjs/core @plexusjs/solid
```

### NextJS Package

<small>With Yarn</small>

```bash
yarn add @plexusjs/core @plexusjs/react @plexusjs/next
```

<small>With NPM</small>

```bash
npm install --save @plexusjs/core @plexusjs/react @plexusjs/next
```
