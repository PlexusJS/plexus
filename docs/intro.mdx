...
<MenuOption
	left={
		<div>
			You can use Plexus's powerful yet simple <mark>built in http client</mark> using the <code>api()</code> Instance!
		</div>
	}
...
<MenuOption
	left={
		<div>
			You can emit and recieve over custom events using the <code>event()</code> Instance from PlexusJS. This is connected to the Plexus Network
			for some cool superpowers!
		</div>
	}
