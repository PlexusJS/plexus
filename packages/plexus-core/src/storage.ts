...
export class StorageInstance {
    ...
    patch(key: string, value: any): void {
        ...
        if (isObject(value)) {
            ...
            ls.setItem(
                this.getKey(key),
                JSON.stringify(
                    Object.values<typeof value>(
                        deepMerge(JSON.parse(item), value)
                    )
                )
            )
        } else {
    ...
    monitor<
        O extends Watchable,
        Type = O extends Watchable<infer T> ? T : never
    >(key: string, object: O) {
        if (key === '' || key === undefined) {
            this.instance().runtime.log(
                'warn',
                `Can't monitor an object with no key`
            )
            return
        }
    ...
    sync(checkValue?: any) {
        ...
        this._internalStore.tracking.forEach(async (object) => {
            ...
            if (key === '' || key === undefined || key === null) {
                this.instance().runtime.log(
                    'warn',
                    `Can't sync an object with no key`
                )
                return
            }
