  ...
  collect(
    ...
    const collectItem = (item: DataType) => {
      ...
      if (
        ...
        if (groups) {
          const groupsNormalized = Array.isArray(groups)
            ? new Set(groups)
            : new Set([groups])
          // if there is a default group, try adding it to the arrayof groups to add
          if (this.config.defaultGroup) {
            const defaultGroupName =
              typeof this.config.defaultGroup === 'string'
                ? this.config.defaultGroup
            ...
            if (!groupsNormalized.has(defaultGroupName as any)) {
              //   groupsNormalized.delete(defaultGroupName as any)
              this.addToGroups(dataKey, defaultGroupName as any)
            }
          }
          ...
          this.addToGroups(
            dataKey,
            Array.from(groupsNormalized) as typeof groups
          )
        }
      }
    }
