...
> {
    ...
    createGroups<Names extends GroupName>(groupNames: [Names, ...Names[]]) {
        ...
        return this as CollectionInstance<
            DataType,
            Groups &
                Map<typeof groupNames[number], PlexusCollectionGroup<DataType>>,
            Selectors
        >
    }
    /**
    ...
    clear(groupNames?: KeyOfMap<Groups> | KeyOfMap<Groups>[]): this {
        ...
        if (groupNames) {
            if (Array.isArray(groupNames)) {
                groupNames.forEach(
                    (groupName) =>
                        this.isCreatedGroup(groupName) &&
                        this.getGroup(groupName).clear()
                )
            } else {
                this.isCreatedGroup(groupNames) &&
                    this.getGroup(groupNames).clear()
            }
        } else {
    ...
    reComputeGroups(groupNames: KeyOfMap<Groups> | KeyOfMap<Groups>[]): this {
        ...
        groupNames.forEach((groupName) =>
            this.reCompute(
                this.getGroup(groupName).value.map(
                    (d) => d[this._internalStore._key]
                )
            )
        )
        return this
    ...
    get selectorsValue() {
        ...
        for (let selector of this._internalStore._selectors.entries()) {
            if (selector[1].value)
                selectors[selector[0] as KeyOfMap<Selectors>] =
                    selector[1].value
        }
        return selectors
    }
